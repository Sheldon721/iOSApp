...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView == self.leftTableView) {
        ...
        leftCell.backgroundColor = LXDColor(244, 244, 244,1);
        //自定义选中时的背景颜色 和字体颜色
        ...
        leftCell.TheSongList.highlightedTextColor = LXDColor(48, 195, 124,1);
        //默认选中第一个单元格
        if (indexPath.row == 0) {
    ...
    else if (tableView == self.rightTableView)
    {
       ...
       // NSLog(@"%@",model.filename);
        ...
        LXDAVPlayer *manager = [LXDAVPlayer manager];
...
- (void)RequestForSongDetails:(NSString *)hash
{
   ...
   [AFNTool GET:[NSString stringWithFormat:@"http://www.kugou.com/yy/index.php?r=play/getdata&hash=%@",hash] parameters:nil success:^(id result) {
       ...
       //播放管理类
       self.manager = [LXDAVPlayer manager];
